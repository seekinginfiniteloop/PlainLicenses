...
class ShameCounter:
    ...
    def __init__(self, config: MkDocsConfig) -> None:
        ...
        if self._instantiated:
            return
        self.shame_map: dict[str, str] = {k.strip().lower(): v.strip().lower() for k, v in config["extra"]["shame_words"].items() if k and v}
...
def on_page_markdown(markdown: str, page: Page, config: MkDocsConfig, files: Files) -> str:
    ...
    if is_license_page(page):
        ...
        if word_count := Counter(
            ...
            logger.debug(f"Found {len(filtered_words)} words in license text for {license_name}. Words: {filtered_words}")
            shamer.shame_counts[license_name] = word_count
            shamer.total_counts.update(word_count)
            shamer.ratios[license_name] = round(((sum(word_count.values()) / len(filtered_words)) * 100), 2) if len(filtered_words) else 0
        ...
        shamer.sort_all()
        logger.debug(f"Shame counts for {license_name}: {shamer.shame_counts[license_name]}")
    return markdown
...
def on_config(config: MkDocsConfig) -> MkDocsConfig:
    ...
    if not (shamer := ShameCounter.instance()):
        shamer = ShameCounter(config)
    if not shamer:
        logger.debug("ShameCounter still not found. Shame words: %s", config["extra"]["shame_words"])
