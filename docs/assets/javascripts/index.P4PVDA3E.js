      ...
      (request2) => from(fetch(request2)).pipe(
        switchMap((response) => {
          ...
          return from(caches.open(type)).pipe(
            ...
            map(() => true),
            tap(() => logger.info(`Asset cached: ${request2.url}`))
          );
        })
      )
    )
    ...
    return from(keys).pipe(
      ...
      tap((deleted) => {
        if (deleted) {
          ...
          logger.info(`Failed to delete cached asset with hash: ${hash}`);
        }
      }),
      catchError((error) => {
      ...
      const loadImage$ = this.loadedImages.has(nextImageName) ? of(this.loadedImages.get(nextImageName)) : this.loadAndPrepareImage(nextImage).pipe(
        ...
        tap(
          (img) => {
            ...
            logger.info(`type: ${typeof img}; Image name: ${nextImageName}; image URL: ${nextImage.src}; height: ${img.naturalHeight}; width: ${img.naturalWidth}`);
          }
        )
      );
...
var cleanCache$ = cleanupCache$(8e3).pipe(tap(() => logger.info("Attempting to clean up cache")), mergeMap(() => merge(cacheAsset$("stylesheets", document.querySelectorAll("link[rel=stylesheet][href*=stylesheets]")), cacheAsset$("javascripts", document.querySelectorAll("script[src*=javascripts]")), cacheAsset$("fonts", document.querySelectorAll("link[rel=stylesheet][href*=fonts]")), cacheAsset$("images", document.querySelectorAll("img[src]")))), tap(() => logger.info("Assets cached")));
...
var pageSubscription$ = navigationEvents$.pipe(map((url) => {
  ...
  matchingConfigs.forEach((config7) => logger.info(`Navigated to ${config7.location}`));
  return matchingConfigs;
}), filter((configs) => configs !== null), switchMap((configs) => {
  const allObservables = configs.flatMap((config7) => config7.observables.map((obs) => obs.pipe(tap(() => logger.info(`Running observables for ${config7.location}`)), catchError((error) => {
    logger.error(`Error in ${config7.location} observables:`, error);
    return EMPTY;
  }))));
