...
function watchLocationHash(location$7) {
  return merge(
...
function watchLocationTarget(location$7) {
  return watchLocationHash(location$7).pipe(
...
function setupInstantNavigation({ location$: location$7, viewport$: viewport$3, progress$: progress$2 }) {
  const config7 = configuration();
  ...
  merge(instant$, history$).subscribe(location$7);
  const document$5 = location$7.pipe(
  ...
  merge(
    document$5.pipe(withLatestFrom(location$7, (_, url) => url)),
...
function mountSearchHiglight(el, { index$: index$2, location$: location$7 }) {
  return combineLatest([
    index$2,
    location$7.pipe(
...
function patchEllipsis({ document$: document$5, viewport$: viewport$3 }) {
  document$5.pipe(
...
var isDevelopment = window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1" || window.location.port === "8000";
var logger = {
  error: (message, ...args) => {
    if (isDevelopment) {
...
ScrollTrigger3.batch = function(targets, vars) {
  var result = [], varsCopy = {}, interval2 = vars.interval || 0.016, batchMax = vars.batchMax || 1e9, proxyCallback = function proxyCallback2(type, callback) {
    var elements = [], triggers = [], delay2 = gsap4.delayedCall(interval2, function() {
  ...
  _toArray2(targets).forEach(function(target) {
    ...
    result.push(ScrollTrigger3.create(config7));
  });
  return result;
};
...
var { location$: location$2 } = window;
...
var mergedUnsubscription$ = (urlFilter3) => {
  const location2 = location$2.pipe(
    filter(urlFilter3)
  );
  const beforeUnload = fromEvent(window, "beforeunload");
  return forkJoin([location2, beforeUnload]);
};
...
var { document$: document$2, location$: location$3 } = window;
if (easterEgg && infoBox) {
  easterEgg.style.display = "block";
}
var infoBoxIsVisible = () => {
  var _a2;
  return (_a2 = infoBox == null ? void 0 : infoBox.open) != null ? _a2 : false;
};
var infoBoxVisibleSubject = new BehaviorSubject(infoBoxIsVisible());
var infoBoxVisible$ = infoBoxVisibleSubject.asObservable();
var hideOverlay = () => {
  if (infoBox) {
    infoBox.close();
...
var allSubscriptions = () => {
  ...
  const eggInteraction$ = createInteractionObservable(
    easterEgg,
    eggFunction
  );
  subscriptions.push(
  ...
  const leaveInfoBoxInteraction$ = createInteractionObservable(
    document,
    eggBoxCloseFunc
  );
  subscriptions.push(
  ...
  const pathObservable$ = location$3.pipe(
    distinctUntilKeyChanged("pathname"),
    filter(
      (location2) => location2.pathname !== "index.html" && location2.pathname !== "/"
    ),
    tap(() => hideOverlay()),
    tap(() => {
  ...
  subscriptions.push(location$3.pipe(
    filter((location2) => location2.pathname === "/" || location2.pathname === "/index.html"),
    skipUntil(location$3.pipe(filter((location2) => location2.hash !== ""))),
    filter(
      (location2) => location2.hash === "#" || location2.hash === "#pt3-hero-section-content"
    )
  ).subscribe(() => {
    history.replaceState(storedLocationState, "", "/");
  }));
};
document$2.pipe(skipUntil(location$3.pipe(filter((location2) => location2.pathname === "/" || location2.pathname === "/index.html")))).subscribe(() => {
  allSubscriptions();
});
var urlFilter = (url) => url.pathname !== "/" && url.pathname !== "/index.html" && url.pathname !== "/#" || url.hostname !== "plainlicense.org" && url.protocol === "https:";
...
function cacheAssets(type, elements) {
...
var cleanCache = () => {
  return getCurrentAssetHashes().pipe(
    switchMap(
      (currentHashes) => openCache().pipe(
        switchMap(
          (cache) => from(cache.keys()).pipe(
            mergeMap(
              (request2) => from(request2).pipe(
                mergeMap((req) => {
                  const { url } = req;
                  const hash = extractHashFromUrl(url);
...
var { document$: document$3, location$: location$4 } = window;
...
var locationChange$ = location$4.pipe(
...
function initImages() {
  ...
  subscriptions2.push(loadFirstImage().pipe(
...
location$4.pipe(skipUntil(location$4.pipe(filter((loc) => loc.pathname === "/" || loc.pathname === "/index.html" || loc.pathname === "/#")))).subscribe(() => {
  initImages();
});
var urlFilter2 = (url) => url.pathname !== "/" && url.pathname !== "/index.html" && url.pathname !== "/#" || url.hostname !== "plainlicense.org" && url.protocol === "https:";
mergedUnsubscription$(urlFilter2).subscribe({
  next: () => {
    subscriptions2.forEach((sub) => sub.unsubscribe());
  }
});

// src/assets/javascripts/licenses/index.ts
var licenses_exports = {};
var { location$: location$5 } = window;
var subscriptions3 = [];
...
location$5.pipe(filter((location2) => location2.pathname.includes("licenses") && location2.pathname.split("/").length >= 4)).subscribe(
  () => {
    ...
    subscriptions3.push(fromEvent(toggle, "change").pipe(
      tap(() => {
        header.setAttribute("aria-expanded", toggle.checked.toString());
      })
    ).subscribe());
    const urlFilter3 = (url) => url.hostname !== "plainlicense.org" && url.protocol === "https:" || !url.pathname.includes("licenses") && url.pathname.split("/").length < 4;
    mergedUnsubscription$(urlFilter3).subscribe({
      next: () => {
        logger.info("Unsubscribing from subscriptions");
        subscriptions3.forEach((sub) => sub.unsubscribe());
      }
    });
  }
);
...
var { document$: document$4, location$: location$6 } = window;
var subscriptions4 = [];
document.documentElement.classList.remove("no-js");
document.documentElement.classList.add("js");
document$4.subscribe(() => {
  bundle_exports;
  feedback_exports;
});
location$6.pipe(filter((location2) => location2.pathname.includes("licenses") && location2.pathname.split("/").length >= 4)).subscribe(() => {
  licenses_exports;
});
location$6.pipe(filter((location2) => location2.pathname === "/")).subscribe(() => {
  hero_exports;
});
...
subscriptions4.push(document$4.pipe(switchMap(() => cleanupCache(8e3).pipe(tap(() => logger.info("Attempting to clean up cache")), mergeMap(() => merge(cacheAssets("stylesheets", styleAssets), cacheAssets("javascripts", scriptAssets), cacheAssets("fonts", fontAssets))), tap(() => {
  logger.info("Assets cached");
}))), tap(() => logger.info("Assets cached"))).subscribe({
  next: () => logger.info("Assets cached successfully"),
  error: (err) => logger.error("Error caching assets:", err),
  complete: () => logger.info("Caching process completed")
}));
subscriptions4.push(document$4.subscribe(() => {
  deleteOldCache().subscribe({
    next: () => logger.info("Old cache deleted"),
    error: (err) => logger.error("Error deleting old cache:", err),
...
subscriptions4.push(document$4.subscribe(() => {
  ...
  tables.forEach((table) => {
    const Tablesort = require_tablesort();
    new Tablesort(table);
  });
}));
subscriptions4.push(location$6.pipe(map((location2) => location2.pathname.split("/")), map((pathArray) => ({
  parent: pathArray[pathArray.length - 2],
  page: pathArray[pathArray.length - 1]
})), filter(({ parent, page }) => parent === "helping" && page === "index.html")).subscribe(() => {
  const script3 = document.createElement("script");
  script3.async = true;
  script3.defer = true;
  script3.src = "https://buttons.github.io/buttons.js";
  document.head.appendChild(script3);
}));
var customUrlFilter = (url) => url.hostname !== "plainlicense.org" && url.protocol === "https:";
mergedUnsubscription$(customUrlFilter).subscribe({
  next: () => {
    subscriptions4.forEach((sub) => sub.unsubscribe());
...
gsap/ScrollTrigger.js:
  ...
  *)
*/
//# sourceMappingURL=index.PW6ZUTOO.js.map
