...
if (typeof window === "undefined") {
  ...
  let customWindow6 = window;
  isDevelopment = customWindow6.location.hostname === "localhost" || customWindow6.location.hostname === "127.0.0.1" || customWindow6.location.port === "8000";
}
var logger = {
  error: (message, ...args) => {
    if (isDevelopment) {
...
function renderSearchDocument(document2, flag) {
  ...
  const url = new URL(document2.location, config6.base);
...
function renderVersion(version) {
  const config6 = configuration();
  const url = new URL(`../${version.version}/`, config6.base);
  return /* @__PURE__ */ h("li", { class: "md-version__item" }, /* @__PURE__ */ h("a", { href: `${url}`, class: "md-version__link" }, version.title, config6.version?.alias && version.aliases.length > 0 && /* @__PURE__ */ h("span", { class: "md-version__alias" }, version.aliases[0])));
}
...
function mountPalette(el) {
  ...
  const meta2 = h("meta", { name: "theme-color" });
  ...
  return defer(() => {
    ...
    ).subscribe((color) => meta2.content = `#${color}`);
...
function setupInstantNavigation({ location$: location$3, viewport$: viewport$5, progress$: progress$2 }) {
  ...
  instant$.pipe(withLatestFrom(viewport$5)).subscribe(([url, { offset }]) => {
  ...
  merge(
    document$5.pipe(withLatestFrom(location$3, (_, url) => url)),
...
function mountSearchResult(el, { worker$, query$ }) {
  ...
  const meta2 = getElement(":scope > :first-child", el);
  ...
  push$.pipe(
    ...
    switch (items.length) {
      /* No results */
      case 0:
        meta2.textContent = value.length ? translation("search.result.none") : translation("search.result.placeholder");
        break;
      /* One result */
      case 1:
        meta2.textContent = translation("search.result.one");
        break;
      /* Multiple result */
      default:
        const count = round(items.length);
        meta2.textContent = translation("search.result.other", count);
    }
  });
...
if (config3.version?.provider === "mike")
...
function watchLicenseHash() {
  ...
  ]).pipe(filter(([first2, second]) => first2.pathname === second.pathname), map(([_, second]) => second), filter((url) => isLicenseHash(url)), tap(handleLicenseHash));
}

// src/assets/javascripts/fetchWorker.ts
var meta = { cacheName: "", urls: [], version: "", worker: "" };
var metaUrl = "assets/javascripts/workers/meta.json";
...
_easeMap.SteppedEase = _easeMap.steps = _globals.SteppedEase = {
  config: function config4(steps, immediateStart) {
    if (steps === void 0) {
      steps = 1;
    }
...
function getContentElements(element) {
  return Array.from(element.querySelectorAll("*")).filter((el) => el !== element && (el.innerHTML.trim() !== "" || el instanceof SVGElement));
}
...
var HeroObservation = class _HeroObservation {
  ...
  setupFirstLoad() {
    ...
    const filteredContent = content.filter((content2) => !emphasisTargets.includes(content2));
...
var pageSubscription$ = navigationEvents$.pipe(map((url) => {
  const matchingConfigs = pageConfigs.filter((config6) => config6.matcher(url));
