...
function throttle(durationSelector, config7) {
  return operate(function(source, subscriber) {
    var _a2 = config7 !== null && config7 !== void 0 ? config7 : {}, _b = _a2.leading, leading = _b === void 0 ? true : _b, _c = _a2.trailing, trailing = _c === void 0 ? false : _c;
    var hasValue = false;
    var sendValue = null;
    var throttled = null;
    var isComplete = false;
...
function throttleTime(duration, scheduler, config7) {
  if (scheduler === void 0) {
    scheduler = asyncScheduler;
  }
...
function watchScript(src) {
  const script3 = h("script", { src });
  return defer(() => {
    document.head.appendChild(script3);
    return merge(
      fromEvent(script3, "load"),
      fromEvent(script3, "error").pipe(
        switchMap(() => throwError(() => new ReferenceError(`Invalid script: ${src}`)))
      )
    ).pipe(
      map(() => void 0),
      finalize(() => document.head.removeChild(script3)),
      take(1)
    );
  });
}
...
function setLocationHash(hash2) {
  const el = h("a", { href: hash2 });
...
var script2 = getElement("#__config");
var config3 = JSON.parse(script2.textContent);
config3.base = `${new URL(config3.base, getLocation())}`;
function configuration2() {
  return config3;
}
function feature2(flag) {
  return config3.features.includes(flag);
}
...
function mountAnnounce(el) {
  ...
  return defer(() => {
    ...
    push$.subscribe(({ hash: hash2 }) => {
      el.hidden = true;
      __md_set("__announce", hash2);
    });
...
function renderSearchDocument(document2, flag) {
  ...
  const url = new URL(document2.location, config7.base);
...
function renderSearchResultItem(result) {
  ...
  const parent = docs.findIndex((doc) => {
    const l = `${new URL(doc.location, config7.base)}`;
    return !l.includes("#");
  });
...
function renderVersion(version) {
  var _a2;
  const config7 = configuration();
  const url = new URL(`../${version.version}/`, config7.base);
  return /* @__PURE__ */ h("li", { class: "md-version__item" }, /* @__PURE__ */ h("a", { href: `${url}`, class: "md-version__link" }, version.title, ((_a2 = config7.version) == null ? void 0 : _a2.alias) && version.aliases.length > 0 && /* @__PURE__ */ h("span", { class: "md-version__alias" }, version.aliases[0])));
}
function renderVersionSelector(versions, active) {
  ...
  return /* @__PURE__ */ h("div", { class: "md-version" }, /* @__PURE__ */ h(
    ...
    active.title,
    ((_a2 = config7.version) == null ? void 0 : _a2.alias) && active.aliases.length > 0 && /* @__PURE__ */ h("span", { class: "md-version__alias" }, active.aliases[0])
...
function inject(next) {
  ...
  return concat(getElements("script", container)).pipe(
    switchMap((el) => {
      const script3 = next.createElement("script");
      if (el.src) {
        for (const name of el.getAttributeNames())
          script3.setAttribute(name, el.getAttribute(name));
        el.replaceWith(script3);
        return new Observable((observer) => {
          script3.onload = () => observer.complete();
        });
      } else {
        script3.textContent = el.textContent;
        el.replaceWith(script3);
        return EMPTY;
      }
    }),
...
function setupInstantNavigation({ location$: location$4, viewport$: viewport$4, progress$: progress$2 }) {
  const config7 = configuration();
  if (location.protocol === "file:")
    return EMPTY;
  const sitemap$ = fetchSitemap(config7.base);
  ...
  const document$4 = location$4.pipe(
  ...
  merge(
    document$4.pipe(withLatestFrom(location$4, (_, url) => url)),
...
function setupSearchHighlighter(config7) {
  const regex = config7.separator.split("|").map((term) => {
  ...
  return (query) => {
    ...
    const match = new RegExp(`(^|${config7.separator}|)(${query.replace(/[|\\{}()[\]^$+*?.-]/g, "\\$&").replace(separator, "|")})`, "img");
...
function setupSearchWorker(url, index$2) {
  ...
  merge(
    ...
    first((active) => active),
    switchMap(() => index$2)
  ).subscribe(({ config: config7, docs }) => worker$.next({
    type: 0 /* SETUP */,
    data: {
      config: config7,
      docs,
...
function setupVersionSelector({ document$: document$4 }) {
  ...
  document$4.pipe(switchMap(() => current$)).subscribe((current) => {
    ...
    if (outdated === null) {
      outdated = true;
      let ignored = ((_a2 = config7.version) == null ? void 0 : _a2.default) || "latest";
...
function mountSearch(el, { index$: index$2, keyboard$: keyboard$2 }) {
  const config7 = configuration();
  try {
    const worker$ = setupSearchWorker(config7.search, index$2);
...
function mountSidebar(el, { header$: header$2, ...options }) {
...
function fetchSourceFactsFromGitLab(base, project) {
  ...
  return zip(
    /* Fetch version */
    requestJSON(`${url}/releases/permalink/latest`).pipe(
      catchError(() => EMPTY),
      // @todo refactor instant loading
      map(({ tag_name }) => ({
        version: tag_name
    ...
    if (feature("navigation.tracking"))
      viewport$4.pipe(
        ...
        if (anchor && anchor.length) {
          const [active] = anchor;
          const { hash: hash2 } = new URL(active.href);
          if (url.hash !== hash2) {
            url.hash = hash2;
            history.replaceState({}, "", `${url}`);
          }
...
function patchEllipsis({ document$: document$4, viewport$: viewport$4 }) {
  document$4.pipe(
...
function fetchSearchIndex() {
  if (location.protocol === "file:") {
    return watchScript(
      `${new URL("search/search_index.js", config4.base)}`
    ).pipe(
    ...
    return requestJSON(
      new URL("search/search_index.json", config4.base)
    );
  }
}
...
var content$ = defer(() => merge(
  ...
  ...getComponentElements("content").map(
    (el) => feature2("search.highlight") ? mountSearchHiglight(el, { index$, location$ }) : EMPTY
  ),
...
var _createPlugin = function _createPlugin2(config7) {
  if (!config7) return;
  config7 = !config7.name && config7["default"] || config7;
  if (_windowExists() || config7.headless) {
    var name = config7.name, isFunc = _isFunction(config7), Plugin = name && !isFunc && config7.init ? function() {
      this._props = [];
    } : config7, instanceDefaults = {
      init: _emptyFunc,
    ...
    if (config7 !== Plugin) {
      ...
      _setDefaults(Plugin, _setDefaults(_copyExcluding(config7, instanceDefaults), statics));
      _merge(Plugin.prototype, _merge(instanceDefaults, _copyExcluding(config7, statics)));
      _plugins[Plugin.prop = name] = Plugin;
      if (config7.targetTest) {
    ...
    config7.register && config7.register(gsap, Plugin, PropTween);
  } else {
    _registerPluginQueue.push(config7);
  }
};
var _255 = 255;
var _colorLookup = {
...
_easeMap.SteppedEase = _easeMap.steps = _globals.SteppedEase = {
  config: function config5(steps, immediateStart) {
    if (steps === void 0) {
      steps = 1;
    }
...
var Animation = /* @__PURE__ */ function() {
  ...
  _proto.revert = function revert(config7) {
    if (config7 === void 0) {
      config7 = _revertConfig;
    }
    var prevIsReverting = _reverting;
    _reverting = config7;
    if (this._initted || this._startAt) {
      this.timeline && this.timeline.revert(config7);
      this.totalTime(-0.01, config7.suppressEvents);
    }
    this.data !== "nested" && config7.kill !== false && this.kill();
...
var Context = /* @__PURE__ */ function() {
  ...
  _proto5.revert = function revert(config7) {
    this.kill(config7 || {});
  };
  return Context2;
}();
var MatchMedia = /* @__PURE__ */ function() {
  ...
  _proto6.revert = function revert(config7) {
    this.kill(config7 || {});
  };
...
var _gsap = {
  registerPlugin: function registerPlugin() {
    ...
    args.forEach(function(config7) {
      return _createPlugin(config7);
    });
  },
...
var Observer = /* @__PURE__ */ function() {
  ...
  _proto.init = function init5(vars) {
    ...
    var id, onStopDelayedCall, dragged, moved, wheeled, locked, axis, self = this, prevDeltaX = 0, prevDeltaY = 0, passive = vars.passive || !preventDefault, scrollFuncX = _getScrollFunc(target, _horizontal), scrollFuncY = _getScrollFunc(target, _vertical), scrollX2 = scrollFuncX(), scrollY2 = scrollFuncY(), limitToTouch = ~type.indexOf("touch") && !~type.indexOf("pointer") && _eventTypes[0] === "pointerdown", isViewport = _isViewport(target), ownerDoc = target.ownerDocument || _doc3, deltaX = [0, 0, 0], deltaY = [0, 0, 0], onClickTime = 0, clickCapture = function clickCapture2() {
...
var ScrollTrigger2 = /* @__PURE__ */ function() {
  ...
  ScrollTrigger4.defaults = function defaults2(config7) {
    if (config7) {
      for (var p in config7) {
        _defaults2[p] = config7[p];
      }
    }
    return _defaults2;
  };
  ...
  ScrollTrigger4.config = function config7(vars) {
...
ScrollTrigger2.batch = function(targets, vars) {
  var result = [], varsCopy = {}, interval2 = vars.interval || 0.016, batchMax = vars.batchMax || 1e9, proxyCallback = function proxyCallback2(type, callback) {
    var elements = [], triggers = [], delay2 = gsap3.delayedCall(interval2, function() {
  ...
  _toArray(targets).forEach(function(target) {
    var config7 = {};
    for (p in varsCopy) {
      config7[p] = varsCopy[p];
    }
    config7.trigger = target;
    result.push(ScrollTrigger2.create(config7));
  });
  return result;
};
...
// src/assets/javascripts/log/index.ts
...
// src/assets/javascripts/feedback/index.ts
var feedback = document.forms.namedItem("feedback");
var feedback$ = of(feedback).pipe(
  tap(() => {
    if (feedback) {
      ...
      feedback.addEventListener("submit", function(ev) {
        ...
        const data = (_a2 = ev.submitter) == null ? void 0 : _a2.getAttribute("data-md-value");
        logger2.info(page, data);
...
var setupTabIconSync$ = () => {
  ...
  const createInteractionStreams = (elements, events) => {
    return events.map(
      (eventName) => elements.flatMap(({ label, iconAnchor }) => [
  ...
  const combined$ = combineLatest([
    ...
    merge(
      ...
      map(([id, eventName]) => ({ id, state: eventStateMap[eventName] })),
      startWith(null)
    )
  ]).pipe(
...
var { document$: document$2, location$: location$2 } = customWindow;
function watchMediaQuery(query) {
  const media = customWindow.matchMedia(query);
  const listener = customWindow.matchMedia(query).addEventListener || customWindow.matchMedia(query).addListener;
...
var prefersReducedMotion$ = watchMediaQuery("(prefers-reduced-motion: reduce)").pipe(startWith(prefersReducedMotion()), shareReplay(1));
...
var isLicensePage = (url) => {
  return url.pathname.endsWith("index.html") && url.pathname.split("/").length === 5 || url.pathname.endsWith("/") && url.pathname.split("/").length === 4;
};
var isLicense = (url) => {
...
var isDev = (url) => {
  return url.hostname === "localhost" && url.port === "8000" || url.hostname === "127.0.0.1" && url.port === "8000";
};
var isOnSite = (url) => {
...
var navigationEvents$ = "navigation" in window ? (
  ...
  merge(
    ...
    location$2.pipe(distinctUntilKeyChanged("pathname"), startWith(getLocation()), shareReplay(1))
  )
);
var watchTable$ = () => {
...
async function windowEvents() {
  const { document$: document$4, location$: location$4, target$: target$2, keyboard$: keyboard$2, viewport$: viewport$4, tablet$: tablet$2, screen$: screen$2, print$: print$2, alert$: alert$2, progress$: progress$2, component$: component$2 } = customWindow;
  const observables = { document$: document$4, location$: location$4, target$: target$2, keyboard$: keyboard$2, viewport$: viewport$4, tablet$: tablet$2, screen$: screen$2, print$: print$2, alert$: alert$2, progress$: progress$2, component$: component$2 };
...
var watchLicenseHashChange$ = () => {
  const hashes = ["#reader", "#html", "#markdown", "#plaintext", "#changelog", "#official"];
  const allLinks = Array.from(document.getElementsByTagName("a"));
  const unhashedLicenseLinks = allLinks.filter(
    (link) => link.href.includes("licenses") && isLicensePage(new URL(link.href)) && !new URL(link.href).hash
  );
  unhashedLicenseLinks.forEach((link) => link.addEventListener("click", () => {
  ...
  const hashedLinks = allLinks.filter(
    (link) => hashes.some((hash2) => link.href.includes(hash2))
  );
  hashedLinks.forEach(
    (link) => link.addEventListener("click", (e) => e.preventDefault())
  );
  const filterHash = (url) => hashes.includes(url.hash);
  const navigationWatcher$ = navigationEvents$.pipe(
    filter(filterHash),
    shareReplay(1)
  );
  const linkWatcher$ = fromEventPattern(
  ...
  const handleHashChange = (url) => {
    ...
    if (inputElement instanceof HTMLInputElement) {
      ...
      (_b = (_a2 = inputElement.parentElement) == null ? void 0 : _a2.querySelector(".tabbed-content")) == null ? void 0 : _b.scrollIntoView({ behavior: "smooth" });
    }
  };
...
var ImageValidationHandler = class {
  static validateNaturalDimensions(img) {
    const result = {
      isValid: false,
      errors: []
    };
  ...
  static validateDOMTestResult(dimensions) {
    const result = {
      isValid: false,
      errors: []
    };
  ...
  static safelyRemoveFromDOM(element) {
    try {
...
var fetchAndCacheAsset$ = (url, cache) => from(fetch(url)).pipe(
  switchMap((response) => {
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
    return from(cache.put(url, response.clone())).pipe(
      tap(() => logger2.info(`Asset cached: ${url}`)),
      map(() => response)
    );
  }),
  catchError((error) => {
    logger2.error(`Error fetching asset: ${url}, error: ${error}`);
...
var getAsset$ = (url, heroCaller = false) => {
  ...
  return openCache$().pipe(
        ...
        switchMap((response) => {
          if (response) {
            ...
            if (cachedHash === requestedHash) {
              logger2.info(`Asset retrieved from cache: ${url}`);
              ...
              return from(cache.delete(url)).pipe(
                ...
                tap(() => logger2.info(`Asset updated: ${url}`))
              );
            }
          } else {
    ...
    catchError((error) => {
      logger2.error(`Error in getAsset$: ${url}`, error);
...
function cacheAsset$(type, elements) {
  const requests = Array.from(elements).map((el) => new Request(extractUrlFromElement(type, el)));
  return from(requests).pipe(
    mergeMap(
      (request2) => from(fetch(request2)).pipe(
        switchMap((response) => {
          if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
          }
          return from(caches.open(type)).pipe(
            switchMap((cache) => from(cache.put(request2, response))),
            map(() => true),
            tap(() => logger2.info(`Asset cached: ${request2.url}`))
          );
        })
      )
    )
  ...
  return of(document).pipe(
    map((doc) => {
      ...
      assetElements.forEach((el) => {
        ...
        const hash2 = extractHashFromUrl(url);
        if (hash2) {
          hashes.add(hash2);
        }
      });
...
var cleanCache = () => {
  const bustCache$ = async (cache, hashe$, keys) => {
    const hashes = await firstValueFrom(hashe$);
    return from(keys).pipe(
      map((requests) => {
        return { requests, hashes };
      }),
      switchMap(({ requests, hashes: hashes2 }) => {
        const filteredRequests = requests.filter((request2) => {
          if (request2 instanceof Request) {
            const { url } = request2;
            const cachedHash = extractHashFromUrl(url);
            return cachedHash && !hashes2.includes(cachedHash);
          }
          return false;
        });
      ...
      mergeMap((request2) => {
        return from(cache.delete(request2));
      }),
      tap((deleted) => {
        if (deleted) {
          logger2.info(`Deleted cached asset with hash: ${hash}`);
        } else {
          logger2.info(`Failed to delete cached asset with hash: ${hash}`);
        }
      }),
      catchError((error) => {
        logger2.error(`Error deleting cached asset: ${error}`);
        return of(false);
      })
    );
  };
...
var cleanupCache$ = (timer2) => {
  return fromEvent(window, "load").pipe(
    delay(timer2),
    switchMap(() => cleanCache()),
    tap((result) => {
      if (result) {
        logger2.info("Cache cleaned successfully.");
      } else {
        logger2.error("Cache cleaning failed.");
      }
    }),
...
var deleteOldCache$ = () => {
  ...
  return from(caches.keys()).pipe(
    ...
    mergeMap((name) => from(caches.delete(name)).pipe(
      tap(() => logger2.info(`Deleted old cache: ${name}`))
    )),
    map(() => true),
...
// src/assets/javascripts/utils/vectorcalc/index.ts
var DimensionValidator = class {
  static validateImageSize(imageSize) {
  ...
  static validateViewportSize(viewportSize) {
  ...
  static validateHeaderHeight(headerHeight) {
  ...
  static validateFocalPoints(focalPoints) {
  ...
  static validateCalculationResult(result) {
...
var Mat3 = class _Mat3 {
  ...
  multiply(other) {
    return new _Mat3(_Mat3.multiplyMatrices(this.values, other.values));
  }
  ...
  inverse() {
    const m = this.values;
    const det = _Mat3.calculateDeterminant(m);
...
var ImageTransformCalculator = class {
  constructor(config7 = {}) {
    this.config = {
      minTranslation: 100,
      maxScale: 1.5,
  ...
  generateEdgeFocalPoint() {
    return {
      x: Math.random() < 0.5 ? -0.2 : 1.2,
      y: Math.random() < 0.5 ? -0.2 : 1.2
    };
  }
  calculateExtendedBounds(bounds) {
  ...
  calculateSpaceTransforms(imageSize, viewportSize, headerHeight = 0) {
  ...
  calculateSafeBounds(imageSize, viewportSize, headerHeight = 0, scale = 1.1) {
    ...
    const corners = [
      ...
      return normToImage == null ? void 0 : normToImage.multiply(Mat3.scale(scale)).transformPoint(normPoint);
    });
    const [minX, maxX, minY, maxY] = ["x", "y"].map((axis) => {
      const values = corners.map((p) => {
        var _a2;
        return (_a2 = p == null ? void 0 : p[axis]) != null ? _a2 : 0;
      });
  ...
  generateTargetPosition(bounds, focalPoint, variance = 0.2) {
    const lerp = (start, end, t) => start + (end - start) * t;
    const clamp3 = (value, min, max) => Math.max(min, Math.min(max, value));
    const addVariance = (target, min, max, variance2) => {
      const range = max - min;
      const offset = (Math.random() - 0.5) * 2 * variance2 * range;
      return clamp3(target + offset, min, max);
    };
    return {
  ...
  calculateAnimationTransforms(imageSize, viewportSize, headerHeight, focalPoints) {
  ...
  calculateAnimationWaypoints(imageSize, viewportSize, headerHeight, focalPoints, variance = 0.2) {
    ...
    const startPos = this.generateTargetPosition(
      ...
      0.8
      // High variance for more randomness
    );
    const positions = [
      startPos,
    ...
    const waypoints = positions.map((pos, i) => {
      const scale = i === positions.length - 1 ? ScaleCalculator.calculateMinimumScale(imageSize, viewportSize, 100, headerHeight).scale : 1.1;
      return {
        position: pos,
...
var ScaleCalculator = class {
  static calculateMinimumScale(imageSize, viewportSize, minOverflow = 100, headerHeight = 0) {
    DimensionValidator.validateImageSize(imageSize);
    DimensionValidator.validateViewportSize(viewportSize);
    DimensionValidator.validateHeaderHeight(headerHeight);
    ...
    logger2.info("Actual Overflow:", actualOverflow);
    return {
      scale: minimumScale,
      scaledDimensions,
      actualOverflow,
  ...
  static calculateOptimalTransformation(imageSize, viewportSize, focalPoints, minOverflow = 100, headerHeight = 0) {
    ...
    const focalPointPositions = ["main", "secondary"].reduce(
      (acc, key) => {
        acc[key] = {
    ...
    const clampedPositions = ["main", "secondary"].reduce(
      (acc, key) => {
        acc[key] = {
  ...
  static generateCSSProperties(calculations) {
    return {
...
var CONFIG2 = {
  travelSpeed: 500,
  // Base speed for debris travel
  glowDuration: 0.2,
  // Duration of each glow pulsation
  glowIntensity: 2,
  // Intensity of the glow effect
  glowPulsations: 3,
  // Number of glow pulsations
  debrisOriginRatio: 0.25
  // Fraction of element width to determine debris origin spread
};
function createDebris(target, maxCount = 16) {
  const debris = [];
  const debrisCount = gsapWithCSS.utils.random(4, maxCount, 1);
  ...
  for (let i = 0; i < debrisCount; i++) {
    ...
    particle.style.width = `${gsapWithCSS.utils.random(1, 4, 1)}px`;
    particle.style.height = `${gsapWithCSS.utils.random(1, 4, 1)}px`;
    particle.style.backgroundColor = Math.random() < 0.5 ? "var(--atomic-tangerine)" : "var(--mindaro)";
    particle.style.borderRadius = `${gsapWithCSS.utils.random(25, 95, 1)}% ${gsapWithCSS.utils.random(25, 90, 1)}% ${gsapWithCSS.utils.random(15, 98, 1)}% ${gsapWithCSS.utils.random(40, 75, 1)}%`;
...
function animateDebris(debris, originRec, headerAdjust = 100) {
  const debrisTimeline = gsapWithCSS.timeline({ paused: true });
  const widthRatio = originRec.width * CONFIG2.debrisOriginRatio;
  const rectRanges = {
  ...
  debris.forEach((particle, idx) => {
    const originX = gsapWithCSS.utils.random(rectRanges.x1, rectRanges.x2, 1);
    const originY = gsapWithCSS.utils.random(rectRanges.y1, rectRanges.y2, 1);
    const destX = gsapWithCSS.utils.random(0, window.innerWidth, 1);
    const destY = gsapWithCSS.utils.random(headerAdjust, window.innerHeight, 1);
    const distance = getDistance(originX, originY, destX, destY);
    const travelDuration = distance / CONFIG2.travelSpeed;
    debrisTimeline.add(
      [
        `debris-${idx}`,
        gsapWithCSS.fromTo(
          particle,
          {
            ...
            y: originY,
            opacity: 1,
            rotation: gsapWithCSS.utils.random(-360, 360),
          ...
          },
          {
            ...
            ease: "power4.out",
            onStart: () => {
              gsapWithCSS.from(particle, {
                filter: `brightness(${CONFIG2.glowIntensity * 4}) blur(${gsapWithCSS.utils.random(1, 4, 1)}px) drop-shadow(0.5px var(--mindaro))`,
                duration: 0.05,
                ease: "power4.out"
              });
            },
            onUpdate: () => {
              gsapWithCSS.to(particle, {
                filter: `brightness(${CONFIG2.glowIntensity}) blur(1px)`,
                duration: CONFIG2.glowDuration,
                yoyo: true,
                repeat: Math.max(Math.round(CONFIG2.glowPulsations * (distance / getMaxDistance(originX, originY, headerAdjust))), 1),
                ease: "sine.inOut"
              });
            },
            onComplete: () => {
...
function getMaxDistance(x, y, headerAdjust) {
...
function animateLetters(el) {
  ...
  const lettersTimeline = gsapWithCSS.timeline({ paused: true });
  ...
  letters.forEach((letter, idx) => {
    ...
    const fromXorY = gsapWithCSS.utils.random(["x", "y"]);
    const originX = fromXorY === "x" ? gsapWithCSS.utils.random(0, windowWidth, 1) : 0;
    const originY = fromXorY === "y" ? gsapWithCSS.utils.random(windowHeight * 0.75, windowHeight, 1) : 0;
    lettersTimeline.add(
      [
        `letter-${idx}`,
        gsapWithCSS.from(letter, {
          opacity: 0,
          scale: gsapWithCSS.utils.random(1.5, 2, 0.1),
          y: originY,
          x: originX,
          rotationX: gsapWithCSS.utils.random(-720, 720),
          rotationY: gsapWithCSS.utils.random(-360, 360),
          rotationZ: gsapWithCSS.utils.random(-720, 720),
          z: gsapWithCSS.utils.random(300, 600, 1),
          visibility: "hidden",
          duration: 1,
          ease: "power4.out",
          onStart: () => {
            gsapWithCSS.set(letter, { visibility: "visible", opacity: 1 });
            gsapWithCSS.to(letter, { filter: "brightness(2) drop-shadow(0 0 2px var(--mindaro))", duration: 1 });
          },
          onComplete: () => {
            debrisTl.play();
            lettersTimeline.add(
              gsapWithCSS.timeline().add(
                gsapWithCSS.to(letter, {
                  color: "var(--atomic-tangerine)",
                  filter: `brightness(${gsapWithCSS.utils.random(2, 4, 0.1)}) blur(2px) drop-shadow(0 0 ${gsapWithCSS.utils.random(1, 4, 0.1)}px var(--mindaro))`,
                  duration: 0.1,
                  ease: "power4.out"
                })
              ).add(
              ...
              `-=${0.5}`
              // Overlap the shimmer effect slightly
            );
          }
        })
      ],
      1 + idx * gsapWithCSS.utils.random(0.05, 0.2, 0.01)
    );
  });
...
function initHeroTextAnimation$() {
  ...
  return from(prefersReducedMotion$).pipe(
    first(),
    switchMap((prefersReducedMotion2) => {
      if (prefersReducedMotion2 && typeof prefersReducedMotion2 === "boolean") {
        logger2.info(`User prefers reduced motion: ${prefersReducedMotion2}`);
        return of(
          tl.add(
            gsapWithCSS.from(["#CTA_header", "#CTA_paragraph", "#hero-primary-button"], {
              opacity: 0,
              duration: 1,
              stagger: 0.2,
              delay: 0.5,
        ...
        if (!ctaContainer || !ctaParagraph || !button) {
          logger2.error("CTA elements not found");
          return of(tl);
        }
        ...
        tl.add(["headerAnimation", animateLetters(ctaContainer)], 0).add(["paragraphAnimation", animateLetters(ctaParagraph)], 0.5).set(button, { opacity: 0 });
        ...
        tl.add(
          [
            ...
            gsapWithCSS.from(button, {
              ...
              ease: "power4.out",
              onStart: () => {
                gsapWithCSS.set(button, { visibility: "visible", opacity: 1 });
                gsapWithCSS.to(button, { filter: "brightness(2) drop-shadow(0 0 2px var(--mindaro))", duration: 1 });
              },
              onComplete: () => {
          ...
          "-=0.5"
          // Overlap button animation slightly with previous animation
        );
        tl.add(
          [
            "buttonTextAnimation",
            gsapWithCSS.to(buttonTextSpans, {
              opacity: 1,
              visibility: "visible",
              duration: 0.5,
              stagger: { amount: 0.1 },
              ...
              onUpdate() {
                this.targets().forEach((el) => {
                  el.style.color = gsapWithCSS.utils.random(
...
var _HeroStateManager = class _HeroStateManager {
  ...
  constructor() {
    ...
    this.homeWatcherSubscription = new Subscription();
    this.optimalWidth$ = new BehaviorSubject(this.getOptimalWidth());
    this.transformCalc = new ImageTransformCalculator();
    this.config = {
    ...
    this.setupHomeWatcher = () => {
      const leftHome$ = navigationEvents$.pipe(
        ...
        takeLast(1),
        // necessary? no. but it's here.
        finalize(() => this.dispose())
        // cleanup
      ).subscribe();
      const homePathWatcher$ = merge(
        this.state$.pipe(
          map((state) => state.isAtHome),
          filter((isAtHome2) => isAtHome2)
        ),
        ...
        tap((loc) => {
          if (loc instanceof URL) {
            ...
            logger2.info(`Home observer completed for ${this.homePath}`);
          }
        }),
        distinctUntilChanged(),
    ...
    this.validateLoadedImage = (img) => Boolean(
      img && img instanceof HTMLImageElement && img.complete && img.naturalWidth > 0 && img.naturalHeight > 0
    );
    this.waitForImageLoad = (img) => {
      if (img.complete) {
        return this.validateLoadedImage(img) ? Promise.resolve(img) : Promise.reject(new Error("Image failed to load properly - zero dimensions"));
      }
      return new Promise((resolve3, reject) => {
        ...
        const onLoad = () => {
          cleanup();
          this.validateLoadedImage(img) ? resolve3(img) : reject(new Error("Image loaded but has invalid dimensions"));
        };
        const onError = () => {
          cleanup();
  ...
  static getInstance() {
    if (!_HeroStateManager.instance) {
      _HeroStateManager.instance = new _HeroStateManager();
    }
    return _HeroStateManager.instance;
  }
  /**
  ...
  loadImage(imageUrl) {
    return getAsset$(imageUrl, true).pipe(
      ...
      catchError((error) => {
        logger2.error("Error loading image:", error, { url: imageUrl });
  ...
  loadAndPrepareImage(imgSettings) {
    ...
    return this.loadImage(imgSettings.src).pipe(
      ...
      switchMap((img) => from(this.waitForImageLoad(img)).pipe(
        catchError((error) => {
  ...
  setImageAttributes(img, imageBlob, imageName, srcset, focalPoints) {
    logger2.info("Setting image attributes for", imageName);
    ...
    logger2.info("Image attributes set for", imageName);
  }
  /**
  ...
  testImageDimensions(img) {
    ...
    try {
      ...
      const styles = window.getComputedStyle(emplacedElement);
      const computedStyle = Object.fromEntries(Array.from(styles).map((key) => [key, styles.getPropertyValue(key)]));
      logger2.info("Image dimensions test results:", {
        natural: {
  ...
  getHeaderRect() {
    ...
    if (tabVisible && tabBox && headerBox) {
      logger2.info("Tab and header are visible");
  ...
  createImageAnimation(img, imageName) {
    ...
    return of(tl).pipe(
      ...
      switchMap(({ timeline: timeline2, prefersReducedMotion: prefersReducedMotion2, textAnimation }) => {
        var _a2;
        ...
        try {
          ...
          const headerHeight = (_a2 = this.state$.value.headerHeight) != null ? _a2 : 0;
          ...
          if (waypoints.length > 0) {
            waypoints.forEach((waypoint, index) => {
              if (Number.isFinite(waypoint.position.x) && Number.isFinite(waypoint.position.y) && Number.isFinite(waypoint.duration)) {
                timeline2.add(
                  [
                    `waypoint${index}`,
                    gsapWithCSS.to(img, {
                ...
                logger2.warn("Invalid waypoint:", waypoint);
              }
            });
          }
          return of(timeline2);
        } catch (error) {
          logger2.error("Error calculating animation:", error);
          return of(timeline2);
        }
      })
    );
  ...
  cycleImage(recalcCurrent = false) {
    ...
    return defer(() => {
      ...
      const loadImage$ = this.loadedImages.has(nextImageName) ? defer(() => {
        const cachedImage = this.loadedImages.get(nextImageName);
        return this.validateLoadedImage(cachedImage) ? of(cachedImage) : throwError(() => new Error("Cached image invalid"));
      }).pipe(
        catchError(() => {
      ...
      return loadImage$.pipe(
        switchMap((newImageElement) => {
          ...
          if (imageInDOM && imageInDOM instanceof HTMLImageElement) {
            try {
              ...
              logger2.error("Error removing image:", error);
            }
          }
        ...
        catchError((error) => {
          logger2.error("Error cycling image:", error);
          return EMPTY;
        })
      );
    });
  ...
  dispose() {
    ...
    _HeroStateManager.instance = null;
  }
  /**
...
_HeroStateManager.createCycler = () => {
  const state = _HeroStateManager.instance || new _HeroStateManager();
  if (!parallaxLayer || !isAtHome()) {
    ...
    state.loadAndPrepareImage(heroImages[0]).pipe(filter((image) => image instanceof HTMLImageElement)).subscribe({
      ...
      error: (error) => {
        logger2.error("Failed to preload image:", error);
  ...
  const loadImages$ = from(state.cycleImage()).pipe(
    catchError((error) => {
      logger2.error("Failed to load initial image:", error);
  ...
  const cycle$ = interval(HERO_CONFIG.INTERVAL).pipe(
    ...
    tap(() => logger2.info("=== Cycle interval triggered ===")),
    mergeMap(() => from(state.cycleImage()), 1),
    map(() => void 0),
    catchError((error) => {
      logger2.error("Failed to cycle image:", error);
      state.updateState({ status: "error" });
      return EMPTY;
    }),
    shareReplay(1)
...
var initCycler = () => {
  var _a2, _b, _c;
  if (!parallaxLayer) {
    logger2.warn("No parallax layer found");
    return;
  }
  logger2.info("Initializing cycler");
  ...
  return interval(HERO_CONFIG.INTERVAL).pipe(
    ...
    switchMap(() => {
      ...
      return from((_b = cyclerRef.current) == null ? void 0 : _b.heroState.cycleImage()).pipe(
        catchError((error) => {
          logger2.error("Error during image cycle:", error);
          return EMPTY;
        })
      );
    }),
...
var eggExit = document.getElementById("egg-box-close");
var FIRST_SCROLL_RATIO = 0.4;
var SECOND_SCROLL_RATIO = 0.6;
var leaveBox$ = fromEvent(document, "click").pipe(filter((ev) => infoBox !== null && (!infoBox.contains(ev.target) || ev.target === eggExit || eggExit.contains(ev.target))), debounceTime(25), tap(() => infoBoxHandler(infoBox)));
var infoBoxHandler = (infoBox2) => {
  if (infoBox2.open) {
    infoBox2.close();
    infoBox2.style.zIndex = "-1";
  } else {
    infoBox2.showModal();
...
var nerfSelectors = (selectors) => {
  return selectors.forEach((selector3) => {
    selector3.addEventListener("click", (ev) => {
      ev.preventDefault();
    });
  });
};
var smoothScroll$ = (el) => {
  ...
  logger2.info(`Setting scroll parameters: target: ${target}, wayPoint: ${wayPoint}, wayPointPause: ${wayPointPause}, duration: ${duration}`);
  const currentScrollBehavior = document.body.style.scrollBehavior;
  if (!(target instanceof HTMLElement)) {
    logger2.error(`Target element ${target} not found within document.`);
    return of(false);
  }
  document.body.style.overflowY = "scroll";
  prefersReducedMotion$.pipe(
    tap((prefersReducedMotion2) => {
      if (prefersReducedMotion2) {
  ...
  const currentScroll = (html == null ? void 0 : html.scrollTop) || 0;
  ...
  const tl = gsapWithCSS.timeline({
    ...
    onUpdate: () => ScrollTrigger2.update()
  });
  ...
  logger2.info(`Current scroll: ${currentScroll}, firstScrollDuration: ${firstScrollDuration}, secondScrollDuration: ${secondScrollDuration}, scrollPositions: ${JSON.stringify(scrollPositions)}`);
  tl.to(html, {
    duration: firstScrollDuration,
    scrollTo: { y: scrollPositions.wayPoint, autoKill: false },
    immediateRender: false,
    onStart: ScrollTrigger2.refresh
  });
  ...
  return fromEventPattern(
    ...
    map(() => true),
    catchError((err) => {
      logger2.error(`Error in Smooth Scroll: ${JSON.stringify(err)}`);
      scrollFallback(target);
      return of(false);
    })
  );
};
var setupAnimation = (selector3, properties) => {
  gsapWithCSS.set(selector3, properties);
};
var createTimeline = (selector3, animations, scrollTrigger) => {
  const timeline2 = gsapWithCSS.timeline({
    scrollTrigger: {
      ...scrollTrigger,
      onRefresh: () => {
        logger2.info(`ScrollTrigger refreshed for ${selector3}`);
      },
      onUpdate: () => {
        logger2.info(`ScrollTrigger updated for ${selector3}`);
      }
    }
  });
...
var createFadeInAnimation = () => {
  ...
  setupInitialStates();
  const makeScrollBatch = (selector3, optionalParams) => {
    const batch = [];
  ...
  const fadeInTargets = (_a2 = document.querySelector("#pt2-hero-content-section")) == null ? void 0 : _a2.querySelectorAll("span");
  ...
  const scrollTriggerRefresh$ = fromEventPattern(
    () => ScrollTrigger2.addEventListener("refresh", () => {
      var _a3;
      (_a3 = [...fadeInTargets ? Array.from(fadeInTargets) : [], ...fadeIn2Targets ? Array.from(fadeIn2Targets) : []]) == null ? void 0 : _a3.forEach((el) => gsapWithCSS.set(el, { clearProps: "translateY" }));
    }),
    (handler) => ScrollTrigger2.removeEventListener("refresh", handler)
  );
  const resize$ = fromEvent(window, "resize").pipe(debounceTime(100), tap(() => ScrollTrigger2.refresh()));
...
var subscribeToAnimation$ = () => {
  const heroSelectors = Array.from(document.querySelectorAll(".hero-target-selector"));
  nerfSelectors(heroSelectors);
  const heroScrollTo$ = defer(() => from(heroSelectors.map((selector3) => fromEvent(selector3, "click").pipe(debounceTime(5), filter((ev) => ev.target === selector3 || selector3.contains(ev.target)), tap(() => {
    if (infoBox.open) {
      infoBox.click();
    }
  })))).pipe(
    mergeMap((evArray) => from(evArray).pipe(
      switchMap((ev) => smoothScroll$(ev.target))
    ))
  ));
  infoBox$.subscribe();
  heroScrollTo$.subscribe();
  prefersReducedMotion$.subscribe((prefersReducedMotion2) => {
    if (prefersReducedMotion2) {
      return;
    } else {
...
var createScript = (src, async2 = true, defer2 = true) => {
  ...
  script3.src = src;
  script3.async = async2;
  script3.defer = defer2;
  document.head.appendChild(script3);
};
  ...
  for (const url of fontUrls) {
    try {
      ...
      if (blob) {
        ...
        logger2.info(`Font loaded from cache: ${url}`);
        URL.revokeObjectURL(blobUrl);
      }
    } catch (error) {
      logger2.error(`Error loading font: ${url}`, error);
    }
  }
};
var preloadStaticAsset$ = () => {
...
var cleanCache$ = cleanupCache$(8e3).pipe(tap(() => logger2.info("Attempting to clean up cache")), mergeMap(() => merge(cacheAsset$("stylesheets", document.querySelectorAll("link[rel=stylesheet][href*=stylesheets]")), cacheAsset$("javascripts", document.querySelectorAll("script[src*=javascripts]")), cacheAsset$("fonts", document.querySelectorAll("link[rel=stylesheet][href*=fonts]")), cacheAsset$("images", document.querySelectorAll("img[src]")))), tap(() => logger2.info("Assets cached")));
var insertAnalytics = () => createScript("https://app.tinyanalytics.io/pixel/ei74pg7dZSNOtFvI", false, true);
var insertButtonScript = () => createScript("https://buttons.github.io/buttons.js", true, true);
var analytic$ = of(insertAnalytics());
var animate$ = document$3.pipe(switchMap(() => subscribeToAnimation$()));
var asset$ = preloadStaticAsset$();
var buttonScript$ = of(insertButtonScript());
var color$ = of(document.body.setAttribute("data-md-color-scheme", "slate"));
var deleteCache$ = deleteOldCache$();
var font$ = from(preloadFonts());
var license$ = setupTabIconSync$();
var licenseView$ = watchLicenseHashChange$();
...
var pageSubscription$ = navigationEvents$.pipe(map((url) => {
  const matchingConfigs = pageConfigs.filter((config7) => config7.matcher(url));
  if (matchingConfigs.length === 0) {
    return null;
  }
  matchingConfigs.forEach((config7) => logger2.info(`Navigated to ${config7.location}`));
  return matchingConfigs;
}), filter((configs) => configs !== null), switchMap((configs) => {
  const allObservables = configs.flatMap((config7) => config7.observables.map((obs) => obs.pipe(tap(() => logger2.info(`Running observables for ${config7.location}`)), catchError((error) => {
    logger2.error(`Error in ${config7.location} observables:`, error);
    return EMPTY;
  }))));
...
gsap/ScrollToPlugin.js:
  ...
  *)
*/
//# sourceMappingURL=index.5F445YJ6.js.map
