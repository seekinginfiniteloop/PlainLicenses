 ...
 */
export const getHeroFilePattern = (
...
export const parseHeroFileName = (filename: string): HeroFile => {
  ...
  if (match && match.groups) {
    return {
      ...
      type: videoExtensions.includes(match.groups.extension)
        ? "video"
        : "image",
...
export async function resolveGlob(
  ...
  try {
    const result = await Promise.resolve(globby(glob, fastGlobOptions)).then(
      (files) => files,
    );
    if (result.length === 0) {
...
export const generatePictureElement = (
  ...
  const sources = Object.entries(sortedImages)
    .map(
      ([ext, { srcset }]) => `<source type="image/${ext}" srcset="${srcset}">`,
    )
    .join("\n");
...
async function resolveFontFiles(): Promise<Partial<PlaceholderMap>> {
  ...
  const fontkey = Object.keys(placeholderMap).find((key) =>
    key.includes("font"),
  );
  const resolvedPlaceholders: Partial<PlaceholderMap> = {};
  ...
  for (const [file, placeholders] of Object.entries(newContent)) {
    try {
      ...
      const newFilename = oldFilename
        .replace(/$_/, "")
        .replace(/_template/, "");
      const newPath = parts.includes(oldFilename)
