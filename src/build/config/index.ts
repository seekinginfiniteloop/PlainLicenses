    ...
    const results = await Promise.allSettled(
      parents.map(async (parent: string) => {
        ...
        for (const ext of availableExtensions) {
          ...
          const flattenedWidths: HeroPaths = filteredChildren.reduce<HeroPaths>((acc: HeroPaths, child: string) => {
            const width: number | undefined = [426, 640, 854, 1280, 1920, 2560, 3840].find((w: number) => child.includes(w.toString()))
            if (width) {
              acc[width as VideoWidth] = child
            }
            return acc
          }, {} as HeroPaths)  // Initialize acc as an empty WidthMap

          const srcset = await generateSrcset(flattenedWidths)
