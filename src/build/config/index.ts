...
const handleReplacementTypes = (value: any) => {
  const filterIt = (s: string) => {
    return s.replace(/"/g, "").replace("docs/", "")
  }
  if (value instanceof Object) {
    return Object.entries(value).map(([k, v]) => {
      return { [filterIt(k)]: handleReplacementTypes(v) }
    })
  } else if (value instanceof Array) {
    return value.map((v) => handleReplacementTypes(v))
  } else {
    return typeof value === "string" ? filterIt(value) : filterIt(value.toString())
  }
}

function replacer(this: any, key: string, value: any): any {
  return Object.fromEntries([handleReplacementTypes(key), handleReplacementTypes(value)])
}
...
export const rawHeroVideos = ${JSON.stringify(videos, replacer, 2)} as const;
