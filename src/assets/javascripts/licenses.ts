...
const getTabElements = (): TabElement[] => {
  ...
  return inputs.map(input => {
    const { id } = input
er(elements => elements.input && elements.label && elements.iconAnchor && elements.iconSVG)
}
...
export const setupTabIconSync$ = () => {
  ...
  const iconAnchorClicks$ = tabElements.map(({ iconAnchor, input }) =>
    fromEvent(iconAnchor, 'click').pipe(
      tap(() => preventDefault        if (!input.checked) {
          input.checked = true
          input.dispatchEvent(new Event('change'))
        }
      })
...
export const setupTabIconSync$ = () => {
  ...
  const combined$ = combineLatest([
    ...
    tap(([selectedId, interaction]) => {
      tabElements.forEach(tab => {
        const state = interaction && tab.input.id === interaction.id ? interaction. selectionMerged$,
    merge(
      hover$, unhover$, focus$, focusVisible$, blur$
    ).pipe(
      map(([id, eventName]) => ({ id, state: eventStateMap[eventName] as "hover" | "normal" | "focus" | "focus-visible" })),
    ...
    tap(([selectedId, interaction]) => {
      tabElements.forEach(tab => {
        const state = interaction && tab.input.id === interaction.id ? interaction.   filter(id => !!id)
    )
  )
  ...
  const combined$ = combineLatest([
    ...
    tap(([selectedId, interaction]) => {
      tabElements.forEach(tab => {
        const state = interaction && tab.input.id === interaction.id ? interaction.      hover$, unhover$, focus$, focusVisible$, blur$
    ).pipe(
      map(([id, eventName]) => ({ id, state: eventStateMap[eventName] as "hover" | "normal" | "focus" | "focus-visible" })),
      startWith(null)
    )
    ...
    tap(([selectedId, interaction]) => {
      tabElements.forEach(tab => {
        const state = interaction && tab.input.id === interaction.id ? interaction.    'focus': 'focus',
    'focus-visible': 'focus-visible',
    'blur': 'normal'
  }
